...
/proc/announce_arrival(mob/living/carbon/human/character, rank, announce_to_ghosts = TRUE)
	...
	if (announce_to_ghosts)
		...
		deadchat_broadcast(span_game(" has arrived at [SSmapping.current_map.map_name] at [span_name(player_area.name)]."), span_game("[span_name(character.real_name)] ([rank])"), follow_target = character, message_type=DEADCHAT_ARRIVALRATTLE) // DARKPACK EDIT - LOADED MAP NAME
